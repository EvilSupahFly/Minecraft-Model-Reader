...
try:
    from amulet_map_editor.debug_file_access import log as debug_log_file
    debug_log = logging.getLogger("amulet.debug-wrapper-compat")
    handler = logging.StreamHandler(debug_log_file)
...
class JavaResourcePackManager(BaseResourcePackManager[JavaResourcePack]):
    ...
    def _load_iter(self) -> Iterator[float]:
        debug_log.debug("=== _load_iter is executing ===")

        import traceback
        debug_log.debug("=== TRACE _load_iter ===\n" + "".join(traceback.format_stack()))

        try:
            canary_path = os.path.join(os.environ.get("LOG_DIR", "/tmp"), "amulet__was_here.txt")
            with open(canary_path, "w") as f:
                f.write("Hello from _load_iter!\n")
            debug_log.debug(f"Canary file written to: {canary_path}")
        except Exception as e:
            log.error(f"Failed to write canary file: {e}")
        ...
        for pack_index, pack in enumerate(self._packs):
            debug_log.debug(f"[LOAD] Resource pack: {getattr(pack, 'root_dir', '<unknown root_dir>')}")
            ...
            if pack.valid_pack and pack.pack_format >= 2:
                ...
                debug_log.debug(f"  Found {len(image_paths)} textures")
                ...
                for image_index, texture_path in enumerate(image_paths):
                    ...
                    if rel_path_list[0] not in UselessImageGroups:
                        ...
                        debug_log.debug(f"[TEXTURE] Cached key: ({namespace}, {rel_path}) -> {texture_path}")

                        if (os.stat(texture_path).st_mtime != self._texture_is_transparent.get(texture_path, [0])[0]):
                            debug_log.debug(f"[TEXTURE] Opening: {texture_path}")
                            try:
                                #im: Image.Image = Image.open(texture_path)
                                im = Image.open(texture_path).convert("RGBA")  # safe fallback
                                im.verify()  # Verifies that it’s not truncated or corrupted
                                debug_log.debug(f"[TEXTURE] Loaded OK: {texture_path}, mode={im.mode}, size={im.size}")
                            except Exception as e:
                                log.error(f"[TEXTURE] Failed to load: {texture_path}, error: {e}")
                            if im.mode == "RGBA":
                ...
                debug_log.debug(f"  Found {len(blockstate_paths)} blockstate files")
                ...
                debug_log.debug(f"  Found {len(model_paths)} model files")
                ...
                for model_index, model_path in enumerate(model_paths):
                    try:
                        ...
                        debug_log.debug(f"    [✓] Indexed model: {model_path}")
                    except Exception as e:
                        log.warning(f"    [×] Failed to index model: {model_path} - {e}")
        ...
        for key, path in model_file_paths.items():
            with open(path) as fi:
                try:
                    ...
                    debug_log.debug(f"  Parsed model: {path}")
                except json.JSONDecodeError:
                    log.error(f"Failed to parse model file file {path}")
                except Exception as e:
                    log.warning(f"  [×] Unexpected error reading model: {path} - {e}")
            ...
            for face_dir in element_faces:
                if face_dir in cube_face_lut:
                    ...
                    if len(texture_path_list) == 2:
                        ...
                        namespace = "minecraft"
                    if "water" in texture_relative_path or "lava" in texture_relative_path:
                        try:
                            debug_log.debug(f"[TRACE] Namespace: {namespace}")
                            debug_log.debug(f"[TRACE] Texture requested: {texture_relative_path}")
                        except Exception as e:
                            log.warning(f"[ERROR] Exception in texture lookup debug: {e}")
                    texture_path = self.get_texture_path(namespace, texture_relative_path)
                    if "water" in texture_relative_path or "lava" in texture_relative_path:
                        debug_log.debug(f"[LOOKUP] Texture requested: ({namespace}, {texture_relative_path}) -> {texture_path}")
    ...
    def _recursive_load_block_model(self, model_path: str) -> dict:
        ...
        if "water" in model_path or "lava" in model_path:
            debug_log.debug(f"[MODEL-LOAD] Requesting model: namespace={namespace}, path={model_path}")

        if (namespace, model_path) in self._model_files:
            if "water" in model_path or "lava" in model_path:
                debug_log.debug(f"[MODEL-LOOKUP] Accessing _model_files[({namespace}, {model_path})]")
            ...
            if "textures" in model:
                if "water" in model_path or "lava" in model_path:
                    debug_log.debug(f"[MODEL-TEXTURE] Model {namespace}:{model_path} has textures: {model['textures']}")
                if "textures" not in parent_model:
                    parent_model["textures"] = {}
                for key, val in model["textures"].items():
                    parent_model["textures"][key] = val
            if "elements" in model:
                debug_log.debug(f"[MODEL-ELEMENT] Model {namespace}:{model_path} has elements: {model['elements']}")
