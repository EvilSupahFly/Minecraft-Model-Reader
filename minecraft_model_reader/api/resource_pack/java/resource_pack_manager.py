...
log.setLevel(logging.DEBUG)  # Only enable DEBUG here
...
class JavaResourcePackManager(BaseResourcePackManager[JavaResourcePack]):
    ...
    def __init__(
        ...
        if isinstance(resource_packs, Iterable):
            ...
            print(f"Iterable self._packs = {self._packs}")
        elif isinstance(resource_packs, JavaResourcePack):
            self._packs = [resource_packs]
            print(f"JavaResourcePack self._packs = {self._packs}")
        else:
    ...
    def _load_iter(self) -> Iterator[float]:
        print(f"LOADED: resource_pack_manager.py: _load_iter()")
        ...
        for pack_index, pack in enumerate(self._packs):
            ...
            yield pack_progress
            print(f"[PACK] Processing pack: {pack}")
            print(f"[PACK ROOT] {pack.root_dir}")
            ...
            if pack.valid_pack and pack.pack_format >= 2:
                ...
                for image_index, texture_path in enumerate(image_paths):
                    ...
                    if rel_path_list[0] not in UselessImageGroups:
                        ...
                        if "water" in rel_path or "lava" in rel_path:
                            print(f"[TEXTURE] Found texture for '{rel_path}' → {texture_path}")
                ...
                for blockstate_index, blockstate_path in enumerate(blockstate_paths):
                    ...
                    if "water" in blockstate_file or "lava" in blockstate_file:
                        print(f"[BLOCKSTATE] Found blockstate '{blockstate_file}' → {blockstate_path}")    
                ...
                for model_index, model_path in enumerate(model_paths):
                    ...
                    if "water" in rel_path or "lava" in rel_path:
                        print(f"[MODEL] Found model for '{rel_path}' → {model_path}")
        ...
        for element in java_model.get("elements", {}):
            ...
            for face_dir in element_faces:
                if face_dir in cube_face_lut:
                    ...
                    #print(f"texture_relative_path = {texture_relative_path}")
                    while isinstance(
